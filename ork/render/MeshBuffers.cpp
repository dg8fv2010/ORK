...
void MeshBuffers::addAttributeBuffer(int index, int size, int vertexsize, AttributeType type, bool norm, int divisor)
{
	int offset;
	if (attributeBuffers.size() > 0) {
		ptr<AttributeBuffer> ab = attributeBuffers[attributeBuffers.size() - 1];
	...
	ptr<AttributeBuffer> a = new AttributeBuffer(index, size, type, norm, NULL, vertexsize, offset, divisor);
	a->setDivisor(divisor);
	attributeBuffers.push_back(a);
}
