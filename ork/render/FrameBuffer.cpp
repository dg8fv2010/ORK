...
ptr<TextureRectangle> FrameBuffer::getFrameBuffer()
{
		vec4<GLint> vp = getViewport();
	int width = vp.z;
	int height = vp.w;
	Texture::Parameters LinearClamp;
#ifdef min
#undef  min
#endif
	LinearClamp.min(LINEAR);
	LinearClamp.mag(LINEAR);
	LinearClamp.wrapS(CLAMP_TO_EDGE);
	LinearClamp.wrapT(CLAMP_TO_EDGE);
	ptr<TextureRectangle> frameTex = new TextureRectangle(width, height, RGBA8, RGBA, PixelType::FLOAT, LinearClamp, Buffer::Parameters(), CPUBuffer(NULL));
	setReadBuffer(COLOR0);
	copyPixels(0,0,0,0,width,height, *frameTex, 0);
	return frameTex;
}

ptr<TextureRectangle> FrameBuffer::getDepthBuffer( )
{
	vec4<GLint> vp = FrameBuffer::getDefault()->getViewport();
	int width = vp.z;
	int height = vp.w;
	Texture::Parameters LinearClamp;
#ifdef min
#undef  min
	...
	LinearClamp.wrapS(CLAMP_TO_EDGE);
	LinearClamp.wrapT(CLAMP_TO_EDGE);
	ptr<TextureRectangle> depthTex = new TextureRectangle(width, height, DEPTH_COMPONENT24, DEPTH_COMPONENT, PixelType::FLOAT, LinearClamp, Buffer::Parameters(), CPUBuffer(NULL));
	setReadBuffer(DEPTH);
	copyPixels(0,0,0,0,width,height, *depthTex, 0);
	return depthTex;
}
